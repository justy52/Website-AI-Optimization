...
export default async function QaExecutionPage({searchParams}:{searchParams:Promise<{validation?:string}>}) {
 ...
 <Panel title="Dedicated fixtures">{human?<form action={qaExecutionAction}><input type="hidden" name="operation" value="fixture"/><label>QA scenario<select className="mx-input" name="faultMode"><option value="NONE">Normal delivery</option><option value="TITLE_MISMATCH">Simulated title delivery mismatch</option></select></label><button className="mx-btn">Create QA fixture</button></form>:null}
