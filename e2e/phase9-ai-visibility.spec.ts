...
test("Phase 9 verified prompts, sampled fixture metrics, honest accounting and immutable history", async ({ page }) => {
  ...
  await page.getByLabel("Workspace name").fill(clientName); await page.getByRole("button", { name: "Create workspace" }).click(); await expect(page.getByRole("heading", { level: 1,  name: "Command Center", exact: true })).toBeVisible();
  await page.goto("/leads"); await page.getByLabel("Company").fill(clientName); await page.getByLabel("Status").selectOption("NEW"); await page.getByLabel("Source").fill("Phase 9 E2E"); await page.getByLabel("Contact name").fill("QA Operator"); await page.getByLabel("Contact email").fill(email); await page.getByLabel("Website URL").fill("https://example.com"); await page.getByRole("button", { name: "Create lead" }).click(); await expect(page.getByRole("heading", { level: 1,  name: clientName })).toBeVisible();
  ...
  await page.getByRole("button", { name: "Start audit" }).click(); await expect(page.getByRole("heading", { name: /Audit/, level: 1 })).toBeVisible(); const auditUrl = page.url(); await page.getByRole("button", { name: "Finalize audit snapshot" }).click(); await expect(page.getByText("Audit snapshot finalized.", { exact: true })).toBeVisible();
  await page.goto(websiteUrl); await page.getByRole("link", { name: "Observed AI Visibility", exact: true }).click(); await expect(page.getByRole("heading", { level: 1,  name: "Observed AI Visibility", exact: true })).toBeVisible(); const visibilityUrl = page.url();
  ...
  await fixture.getByRole("link").first().click(); await expect(page.getByRole("heading", { level: 1,  name: "AI Visibility Observation", exact: true })).toBeVisible(); await expect(page.locator("main")).toContainText("ai-vis-parser-v1.1"); await expect(page.locator("main")).toContainText("Synthetic QA fixture"); const historyUrl = page.url();
  ...
  await expect(page.getByTestId("visibility-run").filter({ has: page.getByRole("heading", { name: "MANUAL: QA manually reviewed consumer surface", exact: true }) })).toContainText("MANUAL");
  await page.goto("/monthly-cycles"); await page.getByLabel("Client").selectOption({ label: `${clientName} - GROWTH` }); await page.getByRole("button", { name: "Create cycle" }).click(); await expect(page.getByRole("heading", { level: 1,  name: "Monthly Fulfillment Cycle" })).toBeVisible();
  ...
  await page.setViewportSize({ width: 390, height: 844 }); await page.goto(visibilityUrl); await expect(page.getByRole("heading", { level: 1,  name: "Observed AI Visibility", exact: true })).toBeVisible(); await expect(page.getByRole("navigation", { name: "Main navigation" })).toBeVisible();
  await page.getByRole("button", { name: "Sign out" }).click(); await expect(page).toHaveURL(/login/); await page.getByRole("button", { name: "Sign in", exact: true }).click(); await page.getByLabel("Email").fill(email); await page.getByLabel("Password").fill(password); await page.getByRole("button", { name: "Enter" }).click(); await expect(page.getByRole("heading", { level: 1,  name: "Command Center", exact: true })).toBeVisible(); await page.goto(historyUrl); await expect(page.locator("main")).toContainText("ai-vis-parser-v1.1");
});
