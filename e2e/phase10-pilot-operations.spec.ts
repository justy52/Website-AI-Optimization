...
test("Phase 10 pilot operations, pause, budget, retention, export and accessibility", async ({ page }) => {
  ...
  await page.goto("/monthly-cycles");await page.getByRole("combobox",{name:"Client",exact:true}).selectOption({label:`${clientName} - GROWTH`});await page.getByRole("button",{name:"Create cycle"}).click();await expect(page.locator("main").getByRole("alert")).toContainText("Automation is paused");
