...
export async function syncUsageLedger(tx: OperationsTx, c: WorkspaceContext, at = new Date()) {
  ...
  await tx.execute(sql`insert into usage_ledger(workspace_id,client_id,website_id,agent_run_id,source_key,provider,category,calls,tool_calls,input_tokens,output_tokens,estimated_cost_usd,actual_cost_usd,state,outcome,occurred_at,measurement_window,source_version)
    select r.workspace_id,r.client_id,r.website_id,r.id,'agent:'||r.id,r.provider,
      ...
      case when r.provider='deterministic' then 0 when jsonb_typeof(r.provider_metadata->'gateway'->'totalCostUsd')='number' then (r.provider_metadata->'gateway'->>'totalCostUsd')::numeric when r.actual_cost_cents>0 then r.actual_cost_cents/100.0 else null end,
    ...
    on conflict(workspace_id,source_key) do update set tool_calls=excluded.tool_calls,
      ...
      estimated_cost_usd=coalesce(excluded.estimated_cost_usd,usage_ledger.estimated_cost_usd),actual_cost_usd=coalesce(usage_ledger.actual_cost_usd,excluded.actual_cost_usd),state=excluded.state,outcome=excluded.outcome,updated_at=now()`);
...
export async function recordPrepareUsage(c: WorkspaceContext, runId: string, provider: string, usage: { inputTokens?: number; outputTokens?: number; estimatedCostCents?: number; actualCostCents?: number; gatewayCostUsd?: number }, database = db) {
  await withTenantContext(database, c, async tx => {
    await tx.update(usageLedger).set({ calls: provider === "deterministic" ? 0 : 1, inputTokens: usage.inputTokens ?? null, outputTokens: usage.outputTokens ?? null,
      estimatedCostUsd: provider === "deterministic" ? "0" : usdFromCents(usage.estimatedCostCents), actualCostUsd: provider === "deterministic" ? "0" : (usage.gatewayCostUsd != null && Number.isFinite(usage.gatewayCostUsd) && usage.gatewayCostUsd >= 0 ? usage.gatewayCostUsd.toFixed(6) : usdFromCents(usage.actualCostCents)), updatedAt: new Date(),
