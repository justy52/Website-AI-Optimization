...
export function GET(request: Request) {
  ...
  return new Response(`<!doctype html><html lang="en"><head><title>${title}</title>${description ? `<meta name="description" content="${description}">` : ""}</head><body><h1>Read-only OPTIQ verification test fixture</h1><p>This is fixed harmless QA content, not a customer website. Its scheduled representations exercise independent verification. No site write is performed.</p><a href="/api/health">Public health</a></body></html>`, { headers: { "Content-Type": "text/html; charset=utf-8", "Cache-Control": "no-store", "X-OPTIQ-QA-Fixture": "true" } });
}
