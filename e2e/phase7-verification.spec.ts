...
async function ensureWork(page: Page, opportunityId: string) {
  if (!(await work(page, opportunityId).count())) {
    await page.getByLabel("Opportunity", { exact: true }).selectOption(opportunityId);
...
async function verify(page: Page, opportunityId: string, result: string) {
  await work(page, opportunityId).getByRole("button", { name: "Verify implementation", exact: true }).click();
  await expect.poll(async () => { await page.reload({ waitUntil: "networkidle" }); return work(page, opportunityId).locator(".mx-chip").allTextContents(); }, { timeout: 120_000, intervals: [2000, 4000] }).toContain(result);
}
...
test("Phase 7 exact packages, independent pass/fail/retry and immutable history", async ({ page, request }) => {
  ...
  const readyAt = Date.now() + 90_000; const correctAt = readyAt + 360_000;
